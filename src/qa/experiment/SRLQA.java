...
public class SRLQA {
    ...
    {
        File[] files = FileUtil.getFilesFromDir(modelDir, ".model.0");
        for (File f : files)
        {
    ...
    public void evaluate(String fileName) throws IOException, FileNotFoundException, InterruptedException {
        ...
        for (int i = 0; i < questionDatas.size(); i++) {
            ...
            while (choices.size() > 0) {
                ...
                String choiceModel = getModel("/Users/samuellouvan/NetBeansProjects/QA/data/processes",currentTrigger);
                double score = 0;
                if (!choiceModel.equalsIgnoreCase(""))
                    score = ClearParserUtil.clearParserPredict("/Users/samuellouvan/NetBeansProjects/QA/data/processes/"+choiceModel, "temp.clearparser", "question"+i+"_"+currentTrigger);
